...
## Narrow verification
Verify with the narrowest complete check first: run the specific test file or the
single command that covers your change, not the whole suite each time. Re-run only
what failed plus what your fix could plausibly affect. One final full-suite run at the
end is enough; repeated full-suite runs add nothing but cost.
