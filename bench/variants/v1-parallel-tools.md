...
## Tool-call batching
When you need several independent pieces of information (multiple file reads, multiple
searches, several independent checks), request them all in a single message as parallel
tool calls instead of one-by-one turns. Never serialize reads that do not depend on
each other's results. Each saved turn saves re-reading the whole context.
